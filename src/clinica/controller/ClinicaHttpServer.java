...
public class ClinicaHttpServer {
    ...
    public void parar() { if (server != null) server.stop(0); }
    ...
    private void handleLogin(HttpExchange ex) throws IOException {
        ...
        if (!"POST".equals(ex.getRequestMethod()))   { respond(ex, 405, "application/json", JsonParser.err("Método inválido")); return; }
    ...
    private void handlePacientes(HttpExchange ex) throws IOException {
        cors(ex);
        if ("OPTIONS".equals(ex.getRequestMethod())) { respond(ex, 204, "text/plain", ""); return; }
        if (!autenticado(ex))                        { respond(ex, 401, "application/json", JsonParser.err("Não autenticado")); return; }
            ...
            switch (method) {
                case "GET" -> {
                    ...
                    if (q != null && !q.isEmpty()) {
                        ...
                        if (p == null) respond(ex, 404, "application/json", JsonParser.err("Não encontrado"));
                        else           respondJson(ex, p.toJson());
                    } else {
                ...
                case "PUT" -> {
                    if (!temId) { respond(ex, 400, "application/json", JsonParser.err("ID obrigatório")); return; }
                    Map<String, String> b = JsonParser.parse(readBody(ex));
                ...
                default -> respond(ex, 405, "application/json", JsonParser.err("Método inválido"));
            }
        } catch (Exception e) {
            respond(ex, 500, "application/json", JsonParser.err(e.getMessage()));
        }
            ...
            switch (method) {
                case "GET" -> {
                    ...
                    if (q != null && !q.isEmpty()) {
                        ...
                        if (m == null) respond(ex, 404, "application/json", JsonParser.err("Não encontrado"));
                        else           respondJson(ex, m.toJson());
                    } else {
            ...
            switch (method) {
                case "GET" -> {
                    if (temId) {
                        ...
                        if (e == null) respond(ex, 404, "application/json", JsonParser.err("Não encontrado"));
                        else           respondJson(ex, e.toJson());
                    } else {
            ...
            switch (method) {
                case "GET" -> {
                    ...
                    if (pacId != null) {
                        ...
                        if (c == null) respond(ex, 404, "application/json", JsonParser.err("Não encontrado"));
                        else           respondJson(ex, c.toJson());
                    } else {
    ...
    private void handleOrdenar(HttpExchange ex) throws IOException {
        ...
        try {
            String arquivo = switch (entidade + "." + campo) {
                case "pacientes.nome"   -> svc.ordenarPacientesPorNome(desc);
                case "medicos.nome"     -> svc.ordenarMedicosPorNome(desc);
                case "consultas.data"   -> svc.ordenarConsultasPorData(desc);
                case "consultas.valor"  -> svc.ordenarConsultasPorValor(desc);
                default -> throw new IllegalArgumentException("Entidade/campo não suportado: " + entidade + "." + campo);
            };
    ...
    private void handleUsuarios(HttpExchange ex) throws IOException {
        ...
        try {
            switch (method) {
                case "GET"    -> respondJson(ex, listJson(svc.listarUsuarios(), u -> u.toString()));
                case "POST"   -> {
                    Map<String, String> b = JsonParser.parse(readBody(ex));
    ...
    private double safeDouble(String s) {
        try { return Double.parseDouble(s == null ? "0" : s.trim()); } catch (Exception e) { return 0; }
    }

    @FunctionalInterface
    interface ToJson<T> { String apply(T t); }
